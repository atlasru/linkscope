...
**Статус: исходники инженерного прототипа, не готовая замена Maltego. Rust-часть не скомпилирована в среде выполнения этого задания: Rust/Cargo отсутствуют. Отрисовка также не проверена: установленный Playwright не имеет браузерного бинарника. Установщики не созданы. Показатели FPS, RAM, времени запуска и анонимности не подтверждены.**
...
Выпускаемые зависимости пока не зафиксированы в `Cargo.lock` / `package-lock.json`: без Cargo и реестра они не генерировались. На первой доверенной сборочной машине разрешить зависимости, проверить лицензии/уязвимости и закоммитить lock-файлы; после этого CI должен использовать `--locked` / `npm ci`.
...
- `cargo test -p linkscope-core`: 12 тестов подготовлены, **не запускались**.
...
- `.github/workflows/check.yml`: предложенные gates для ядра и `cargo check` на трёх ОС. Workflow не запущен.
